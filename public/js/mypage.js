    ...
    classifyReservations() {
      ...
      this.upcomingReservations = this.reservations.filter(r => {
        ...
        if (!r.reservation_date || !r.start_time) {
          console.warn('[MyPage] Invalid reservation data:', r);
          return false;
        }
        
        const reservationDate = new Date(`${r.reservation_date} ${r.start_time}`);
        return reservationDate >= now && (r.status === 'CONFIRMED' || r.status === 'LOCKED' || r.status === 'PENDING');
      }).sort((a, b) => {
      ...
      console.log('[MyPage] Classification:', {
        upcoming: this.upcomingReservations.length,
        past: this.pastReservations.length,
        upcomingList: this.upcomingReservations.map(r => r.reservation_code),
        pastList: this.pastReservations.map(r => r.reservation_code)
      });
    }
    ...
    initLottieAnimation() {
      ...
      if (container) {
        this.lottieAnimation = lottie.loadAnimation({
          ...
          loop: true,
          autoplay: true,
          path: CONFIG.ASSETS.LOTTIE.LOADING_DOG
        });
      }
    }
